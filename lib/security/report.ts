...
function determineBadges(
  ...
  if (score.overall >= 85 && criticalCount === 0 && highCount === 0) {
    badges.push({
      ...
      issuer: "Alian-Structure Scanner",
      issuedAt: now,
      score: score.overall,
    });
  }
  ...
  if (allRequiredPassed && compliance.length > 0) {
    badges.push({
      ...
      issuer: "Alian-Structure Compliance",
      issuedAt: now,
    });
  }
  ...
  if (
    ...
    badges.push({
      type: "certified",
      label: "Alian-Structure Certified",
...
export function exportReportAsMarkdown(report: SecurityReport): string {
  ...
  lines.push("---");
  lines.push(`*Report generated by Alian-Structure Security Scanner*`);
